...
# 0.input relation 1.encoder 2.attention 3.multi_att 4.forward
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, boxes, rela_labels_mask, tgt, src_mask, tgt_mask):
        "Take in and process masked src and target sequences."
        return self.decode(self.encode(src, boxes, rela_labels_mask, src_mask), src_mask,
                            tgt, tgt_mask)

    def encode(self, src, boxes, rela_labels_mask, src_mask):
        return self.encoder(self.src_embed(src), boxes, rela_labels_mask, src_mask)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, boxes, rela_labels_mask, mask):
        "Pass the input (and mask) through each layer in turn."
        for layer in self.layers:
            x = layer(x, boxes, rela_labels_mask, mask)
        return self.norm(x)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, boxes, rela_labels_mask, mask):
        ...
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, boxes, rela_labels_mask, mask))
...
class MultiHeadedAttentionWithRelations(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)
        self.rela_embed_linear_k = nn.Sequential(nn.Linear(1000, 64),
                                     nn.ReLU(),
                                     nn.Dropout(0.1))
        self.rela_embed_linear_v = nn.Sequential(nn.Linear(1000, 64),
                                     nn.ReLU(),
                                     nn.Dropout(0.1))
        self.linears_boxes = nn.Sequential(nn.Linear(200, self.d_k),
                                    nn.ReLU(),
                                    nn.Dropout(0.1))
        self.Value_weight = nn.Linear(3 * 64, 2)

    def forward(self, query, key, value, boxes, rela_labels_mask, mask=None):
        ...
        boxes_embedding = utils.BoxRelationalEmbedding(boxes) #B*N*4-->B*N*N*4/64
    ...
    def relative_attention_logits(self, query, key, relation, boxes):
        ...
        # relation & boxes: [batch, num queries, num kvs, depth].
        ...
        # q_tr_tmatmul_t is [batch, heads, num queries, num kvs]
        ...
        # * [batch, num kvs, depth, heads]
        # = [batch, num queries, num kvs, heads]
        # key_tr_tmatmul_t is [batch, heads, num queries, num kvs]
        # For each batch and query, we have a query vector per head
        # We take its dot product with the relation vector for each kv.
        ...
        # * [batch, num queries, depth, num kvs]
        # = [batch, num queries, heads, num kvs]
        # q_boxes_tmatmul_t is [batch, heads, num queries, num kvs]
        # For each batch and query, we have a query vector per head.
        # We take its dot product with the relation vector for each kv.
        q_t = query.permute(0, 2, 1, 3)
        boxes_t = boxes.transpose(-2, -1)
        q_boxes_t_matmul = torch.matmul(q_t, boxes_t)
        q_boxes_tmatmul_t = q_boxes_t_matmul.permute(0, 2, 1, 3)

        # [batch, heads, num queries, num kvs]
        return (qk_matmul + q_tr_tmatmul_t + key_tr_tmatmul_t + q_boxes_tmatmul_t) / math.sqrt(query.shape[-1])
    ...
    def relative_attention_values(self, weight, value, relation, boxes):
        ...
        # relation & boxes: [batch, num queries, num kvs, depth].
        # w_v_matmul is [batch, heads, num queries, depth]
        w_v_matmul = torch.matmul(weight, value)
        ...
        # * [batch, num queries, num kvs, depth]
        # = [batch, num queries, heads, depth]
        # w_boxes_matmul_t is [batch, heads, num queries, depth]
        w_boxes_matmul = torch.matmul(w_t, boxes)
        w_boxes_matmul_t = w_boxes_matmul.permute(0, 2, 1, 3)

        # Gating Mechanism
        f = torch.cat((w_v_matmul, w_tr_matmul_t, w_boxes_matmul_t), dim=-1)
        tr = torch.sigmoid(self.Value_weight(f))
        tr_wgt = tr[:,:,:,0].unsqueeze(-1)
        boxs_wgt = tr[:,:,:,1].unsqueeze(-1)

        return w_v_matmul + tr_wgt * w_tr_matmul_t + boxs_wgt * w_boxes_matmul_t

    # Adapted from The Annotated Transformer
    def attention_with_relations(self, query, key, value, relation_k, relation_v, boxes, mask=None, dropout=None):
        "Compute 'Scaled Dot Product Attention'"
        d_k = query.size(-1)
        scores = self.relative_attention_logits(query, key, relation_k, boxes)
        if mask is not None:
            scores = scores.masked_fill(mask == 0, -1e9)
...
class TransformerModel_Triplet(CaptionModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, N=6,
        ...
        c = copy.deepcopy
        attn = MultiHeadedAttentionWithRelations(h, d_model)
        ...
        model = EncoderDecoder(
            ...
            Decoder(DecoderLayer(d_model, c(attn_decoder), c(attn_decoder),
                                 c(ff), dropout), N),
    ...
    def __init__(self, opt):
        ...
        self.embed_rela = build_embeding_layer(self.vocab_size + 1, 1000, self.drop_prob_lm)
        self.init_weights()

        self.proj_obj = nn.Sequential(
            *[nn.Linear(self.input_encoding_size + self.sg_label_embed_size * self.opt.num_obj_label_use,
                        self.input_encoding_size), nn.ReLU(), nn.Dropout(0.5)])
    ...
    def _prepare_graph_features(self, sg_data, att_feats):
        """
        prepare sg features
        """
        ...
        att_feats = self.proj_obj(torch.cat([att_feats, obj_embed], dim=-1)) + att_feats
        rela_labels_mask = self.embed_rela(rela_labels_mask)

        return  rela_labels_mask, att_feats
    ...
    def prepare_core_args(self, sg_data, att_feats, att_masks, boxes, seq=None ):
        ...
        rela_labels_mask, att_feats = self._prepare_graph_features(sg_data, att_feats)
        ...
        core_args = [ att_feats, boxes, rela_labels_mask, seq, att_masks, seq_mask]
        return core_args
    ...
    def _forward(self, sg_data, att_feats, boxes, seq, att_masks=None):
        ...
        att_feats, boxes, rela_labels_mask, seq, att_masks, seq_mask = self.prepare_core_args(sg_data, att_feats, att_masks, boxes, seq)

        # make seq_per_img copies of the encoded inputs:  shape: (B, ...) => (B*seq_per_image, ...)  seq, seq_mask has original B*seq_per_image,
        core_args0 = [att_feats, boxes, rela_labels_mask, att_masks ]

        core_args1 = expand_feats(core_args0, self.seq_per_img)
        att_feats = core_args1[0]
        boxes = core_args1[1]
        rela_labels_mask = core_args1[2]
        att_masks = core_args1[3]
        out = self.model(att_feats, boxes, rela_labels_mask, seq, att_masks, seq_mask)
    ...
    def _sample_beam(self, sg_data, att_feats, boxes, att_masks=None, opt={}):
        ...
        att_feats, boxes, rela_labels_mask, seq, att_masks, seq_mask  = self.prepare_core_args(sg_data, att_feats, att_masks, boxes)

        memory = self.model.encode(att_feats, boxes, rela_labels_mask, att_masks)
    ...
    # Used in model ensemble
    def GET_EncoderData(self, sg_data, att_feats, boxes, att_masks=None):
