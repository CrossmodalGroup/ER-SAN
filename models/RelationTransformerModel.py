...
def sparse_attention(query, key, value, k=8, mask=None, dropout=None):
    "Compute 'Scaled Dot Product Attention'"
    ...
    if k:
        ...
        vk = torch.unsqueeze(v[:, :, :, -1],3).expand_as(scores)
...
class sparse_MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        "Take in model size and number of heads."
        super(sparse_MultiHeadedAttention, self).__init__()
        assert d_model % h == 0
        # We assume d_v always equals d_k
        self.d_k = d_model // h
        self.h = h
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 2) Apply attention on all the projected vectors in batch.
        x, self.attn = sparse_attention(query, key, value, k=8, mask=mask,
                                 dropout=self.dropout)
...
def sparse_box_attention(query, key, value,  box_relation_embds_matrix, mask_box, k=16, mask=None, dropout=None):
 ...
 #   scaled_dot = scaled_dot.view(key.size()[0] * 8, key.size()[2], key.size()[3])

    if k > key.size()[2]:  #B*H*
        k = key.size()[2]
    if k:
        v, _ = torch.topk(scaled_dot, k)
        vk = torch.unsqueeze(v[:, :, :, -1],3).expand_as(scaled_dot)
        #vk = v[:, :, :, -1].unsqueeze(3).expand_as(scaled_dot)

        mask_k = torch.lt(vk, scaled_dot) # vk > scaled_dot return 1 这里vk在前， scaled 在后
       # mask_box = mask_box.view(key.size()[0] * 8, key.size()[2], key.size()[3])
        mask_k = torch.max(mask_k, mask_box) # add mask_k and mask_box

        scaled_dot = scaled_dot.masked_fill(mask_k == 0, -1e18) #mask==0, set -inf drop
    ...
    #w_mn = torch.log(torch.clamp(w_g, min = 1e-6)) + w_a # 输入一个张量，张量输出的夹紧区间[min, max]
...
class sparse_BoxMultiHeadedAttention(nn.Module):
   ...
   # Self-attention layer with relative position weights.
    ...
    def __init__(self, h, d_model, trignometric_embedding=False, legacy_extra_skip=False, dropout=0.1):
        "Take in model size and number of heads."
        super(sparse_BoxMultiHeadedAttention, self).__init__()
    ...
    def forward(self, input_query, input_key, input_value, input_box, mask=None):
        ...
        relative_geometry_embeddings, mask_box = utils.BoxRelationalEmbedding(input_box, trignometric_embedding= self.trignometric_embedding)
        ...
        # 2) Apply attention on all the projected vectors in batch.
        x, self.box_attn = sparse_box_attention(query, key, value,  relative_geometry_weights, mask_box, k=16, mask=mask,
                                 dropout=self.dropout)
...
class RelationTransformerModel(CaptionModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, N=6,
                   ...
                   trignometric_embedding=False, legacy_extra_skip=False):
        "Helper: Construct a model from hyperparameters."
        c = copy.deepcopy
        bbox_attn = sparse_BoxMultiHeadedAttention(h, d_model, trignometric_embedding, legacy_extra_skip)
        #bbox_attn = sparse_MultiHeadedAttention(h, d_model)
        attn = sparse_MultiHeadedAttention(h, d_model)
