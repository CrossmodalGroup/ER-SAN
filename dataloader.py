...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        if self.use_box:
            sg_batch, boxes_batch, att_batch, label_batch, gts, infos = \
                zip(*sorted(zip(sg_batch, boxes_batch, att_batch, np.vsplit(label_batch, batch_size), gts, infos), key=lambda x: 0, reverse=True))
        else:
            sg_batch, att_batch, label_batch, gts, infos = \
                zip(*sorted(zip(sg_batch, att_batch, np.vsplit(label_batch, batch_size), gts, infos), key=lambda x: 0, reverse=True))
        ...
        data['sg_data'] = {k: v for k, v in vrg_batch_data.items() if k != 'verb_labels'}
    ...
    def batch_sg(self, sg_batch, max_att_len):
        # batching object and relationship data
        # rela_batch: semantic rela;  rela_geometry_batch: geometry rela(pre-stored, can also be computed in utils)
        ...
        # sematic rela mask (sematic graph)
        sg_data['rela_labels_mask'] = np.zeros([sg_data['obj_labels'].shape[0], sg_data['obj_labels'].shape[1], sg_data['obj_labels'].shape[1]],dtype='int')
        ...
        ##### geometry rela ##### we can also obtain the geometry graph in utils.py
        ...
        #     sg_data['geometry_rela_edges'][i, :rela_geometry_batch[i]['edges'].shape[0]] = rela_geometry_batch[i]['edges']
        #
        # # geometry rela mask (geometry graph)
